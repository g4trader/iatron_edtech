...
export async function completeDiagnostic(formData:FormData){const assessmentId=String(formData.get('assessmentId'));await finishAssessment(assessmentId);revalidatePath('/app');revalidatePath('/app/assessment/history');revalidatePath('/app/assessment/coverage');revalidatePath('/app/assessment/result');revalidatePath('/app/plan');redirect(`/app/assessment/result?id=${assessmentId}`);}
